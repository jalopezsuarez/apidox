(function($)
{
	...
	var allEndpoints = $('li.endpoint'), allEndpointsLength = allEndpoints.length;
	...
	// Toggle an endpoint
	$('li.endpoint > h3.title').click(function()
	{
	...
	$('#toggle-endpoints').click(function(event)
	{
		...
		var endpoints = $('ul.methods:not(:visible)'), endpointsLength = endpoints.length, action;
	...
	$('#toggle-methods').click(function(event)
	{
		...
		var methodForms = $('ul.methods form:not(:visible)'), // Any hidden
																// method forms
		...
		{
			var methodLists = $('ul.methods:not(:visible)'), // Any hidden
																// methods
		...
		else
		{
			...
			var visibleMethodForms = $('ul.methods form:visible'), visibleMethodFormsLength = visibleMethodForms.length;
	...
	$('li.list-methods a').click(function(event)
	{
		...
		var endpoint = $(this).closest('li.endpoint'), methods = $('li.method form', endpoint);
	...
	/*
	 * Try it! button. Submits the method params, apikey and secret if any, and
	 * apiName
	 */
	$('li.method form').submit(function(event)
	{
		...
		var params = $(this).serializeArray(), apiKey =
		{
			name : 'apiKey',
		...
		{
			name : 'apiSecret',
			value : $('input[name=secret]').val()
		}, apiName =
		{
			name : 'apiName',
			value : $('input[name=apiName]').val()
		};
		...
		if ($('pre.response', resultContainer).length === 0)
		{
			...
			// Clear results link
			$(document.createElement('a')).text('Clear results').addClass('clear-results').attr('href', '#').click(function(e)
			{
				e.preventDefault();

				var thislink = this;
				$('.result', self).slideUp(function()
				{
					$(this).remove();
					$(thislink).remove();
				});
			...
			// Response
			resultContainer.append($(document.createElement('h4')).text('Response Body').append($(document.createElement('a')).text('Select body').addClass('select-all').attr('href', '#').click(function(e)
			{
				e.preventDefault();
